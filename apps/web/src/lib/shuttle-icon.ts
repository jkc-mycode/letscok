...
function shuttleSvg(theme: (typeof ICON_THEME)[IconApp]): string {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="none">
  <path d="M36 62 L20 24 Q50 12 80 24 L64 62 Z" fill="${theme.feather}"/>
  <path d="M43 62 L34 19" stroke="${theme.stroke}" stroke-width="2.4" stroke-linecap="round"/>
  <path d="M50 62 L50 15.5" stroke="${theme.stroke}" stroke-width="2.4" stroke-linecap="round"/>
  <path d="M57 62 L66 19" stroke="${theme.stroke}" stroke-width="2.4" stroke-linecap="round"/>
  <rect x="34" y="58" width="32" height="9" rx="4.5" fill="${theme.band}"/>
  <path d="M36 66 a14 14 0 0 0 28 0 Z" fill="${theme.feather}"/>
</svg>`;
}

// satori(next/og)는 인라인 SVG 엘리먼트를 그대로 못 받아서 data URI 이미지로 넘긴다
export function shuttleDataUri(app: IconApp): string {
  return `data:image/svg+xml;base64,${Buffer.from(shuttleSvg(ICON_THEME[app])).toString('base64')}`;
}
