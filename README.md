...
| `/history` | 운영진 | 지난 모임 기록·개인 전적 (패스코드 필요) |
...
- **중복 대기** — 잔여 인원(4로 안 나눠질 때)을 게임 중인 사람과 미리 조합. [게임 중 포함] 토글로 수동 조합도 가능
- **선수 교체** — 부상·급한 일로 한 명만 바꿀 때 게임을 갈아엎지 않고 교체 (타이머·순서 유지)
- **운영진 인증** — 공유 패스코드(localStorage 유지). 모임 종료 시 자동 로그아웃
- **모임 히스토리** — 지난 모임 기록·개인 전적(출석/게임 수·함께 뛴 사람 top 5), 운영진 전용
